...
const registerUser = async (firstname, lastname, username, password, role) => {
...
const login = async (username, password) => {
    ...
    if (!passwordVerified)
        throw { message: 'Invalid credentials', code: 401 };
    const access_token = jwt.sign({ id: userFound._id, scopes: userFound.role },jwtKey,{expiresIn: '1h'});
    const refreshToken = randToken.uid(256);
    const newAccess = new Access({refreshToken,userId:userFound._id});
    await newAccess.save();
    return {access_token, scopes: userFound.role, expiresIn: '1h', refresh_token:refreshToken};
}
...
const logout = async (refreshToken, userId) => {
    const accessFound = await Access.deleteOne({refreshToken, userId});
    return accessFound;
}
